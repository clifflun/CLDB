...
st.title('Carvalho Lab Variant Database (v1.1.0)')
		...
		with col6_cnv: 
			with st.expander('Breakpoint Filters '):	
				...
				if st.checkbox('CNV overlaps with haploinsufficient genes  ',
					help='check if CNV overlaps with haploinsufficient genes'):
					pHaplo_cnv = True
				else: 
					pHaplo_cnv = False

				if st.checkbox('CNV overlaps with triplosensitive genes  ',
					help='check if CNV overlaps with triplosensitive genes'):
					pTriplo_cnv = True
				else: 
					pTriplo_cnv = False
